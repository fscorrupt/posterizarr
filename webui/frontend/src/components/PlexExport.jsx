...
const ActionMenu = ({ item, onAction, t }) => {
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-1 w-56 bg-theme-card border border-theme-border rounded-lg shadow-xl z-50 overflow-hidden">
          <div className="py-1">
            <button
              onClick={(e) => {
                e.stopPropagation();
            ...
            >
              ...
              {t("plexActions.refreshMetadata", "Refresh Metadata")}
            </button>
            <button
              onClick={(e) => {
                e.stopPropagation();
                setIsOpen(false);
                onAction("analyze_item", item);
              }}
              className="w-full text-left px-4 py-2.5 text-sm text-theme-text hover:bg-theme-hover flex items-center gap-2"
            >
              <Activity className="w-3.5 h-3.5" />
              {t("plexActions.analyzeItem", "Analyze Item")}
            </button>
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                setIsOpen(false);
                onAction("scan_library", item);
              }}
              className="w-full text-left px-4 py-2.5 text-sm text-theme-text hover:bg-theme-hover flex items-center gap-2"
            >
              <FolderSearch className="w-3.5 h-3.5" />
              {t("plexActions.scanLibrary", "Scan Library")}
            </button>
            <button
              onClick={(e) => {
                e.stopPropagation();
                setIsOpen(false);
                onAction("empty_trash", item);
              }}
              className="w-full text-left px-4 py-2.5 text-sm text-theme-text hover:bg-theme-hover flex items-center gap-2 text-red-400"
            >
              <Trash2 className="w-3.5 h-3.5" />
              {t("plexActions.emptyTrash", "Empty Trash")}
            </button>
          </div>
        </div>
      )}
...
// Helper to safely format DB dates
const formatDbDate = (dateStr) => {
    if (!dateStr) return "-";
    // Replace space with T to ensure ISO format for consistent parsing
...
function PlexExport() {
  ...
  // Plex Actions Handler
  const handlePlexAction = async (action, item) => {
    showInfo(t("plexActions.actionStarted", "Sending command to Plex..."), 2000);

    try {
      const response = await fetch(`${API_URL}/plex/action`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action: action,
          rating_key: item.rating_key || item.media_id,
  ...
  // Helper to get unique library names for the filter pills
              ...
              <tr>
                {activeTab === "library" ? (
                  <>
                    ...
                    <HeaderCell {...headerProps(t("plexExport.ratingKey", "ID"), "rating_key", "w-32")} />
                      ...
                      <>
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2">
                            <ActionMenu item={item} onAction={handlePlexAction} t={t} />
                          </div>
                        </td>
                      </>
                    ) : (
              ...
              {activeTab === "library" ? (
                <>
                  ...
                  <div className="grid gap-4">
                    ...
                    <div>
                        <DetailRow label={t("plexExport.ratingKey", "Rating Key")} value={selectedItem.rating_key} monospace />
                    </div>
