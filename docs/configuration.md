...
2. Update the following variables with your API keys and preferences [my personal config](https://github.com/fscorrupt/posterizarr/blob/main/MyPersonalConfig.json):
    ...
    - `EnableSquareArt`: Set to `true` to enable automatic generation of `square.jpg` assets (1000x1000 resolution).
    - `fontAllCaps`: Set to `true` for all caps text, else `false`.
    - `AddBorder`: Set to `true` to add a border to the image.
    - `AddText`: Set to `true` to add text to the image.
    - `AddTextStroke`: Set to `true` to add stroke to text.
...
## Square Art Changelog

**What was changed/implemented:**
- **Core Automation**: Integrated a new `Invoke-SquareArtCreation` pipeline that runs in parallel with Posters and Backgrounds. It fetches square artwork (1000x1000) from TVDB (Type 5/25), Fanart (cdart/hdtvlogo), Plex, and Jellyfin/Emby (Box).
- **Kometa Support**: Downloads the artwork locally as `square.jpg` directly alongside your standard assets (or within the library folders) so Kometa can automatically pick it up.
- **EXIF Verification**: Ensures we don't accidentally re-download images that Posterizarr previously generated by verifying EXIF tags during Plex/Jellyfin fallback fetches.
- **Maintenance Integration**: Added square art support to the `BackupRestore.ps1` module and `AssetReset.ps1` module.
- **Web UI & Asset Builder**: The configuration tab dynamically handles `SquareArtOverlayPart` settings. The new unified Asset Builder (`Blueprints.jsx`) natively supports customizing and previewing your Square Art (1:1 canvas size).
- **Security Check**: Fixed a potential Path Traversal vulnerability in the `/api/assets/save` backend route.
